...
download_month <- function(year = 2015, month) {
  ...
  dst <- paste0("data-raw/flights/", "2015-", month, ".csv")
  file.rename(src, dst)
}

months <- 1:12
needed <- paste0("2015-", months, ".csv")
missing <- months[!(needed %in% dir("data-raw/flights"))]

lapply(missing, download_month, year = 2015)
...
save(flights, file = "data/flights-2015.rda", compress = "bzip2")
